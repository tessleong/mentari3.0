...
impl From<CaptureParams> for listener::actors::SessionParams {
    fn from(value: CaptureParams) -> Self {
        ...
        Self {
            ...
            expected_speaker_count: value.expected_speaker_count,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn preserves_confirmed_speaker_count_for_listener_session() {
        let mut params = capture_params("https://api.deepgram.com/v1", "nova-3-general");
        params.expected_speaker_count = Some(4);

        let session: anlg_transcription_core::listener::actors::SessionParams = params.into();

        assert_eq!(session.expected_speaker_count, Some(4));
    }
