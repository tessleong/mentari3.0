...
pub fn setup<R: Runtime>(app: &AppHandle<R>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let callback = anlg_detect::new_callback(move |event| {
        let env = env.clone();
...
fn handle_mic_started<E: Env>(
    env: &E,
    state: &ProcessorState,
    apps: Vec<anlg_detect::InstalledApp>,
) {
