...
export const commands = {
  ...
  async inspectCustomModelPath(
    path: string,
  ): Promise<Result<CustomSttModelInfo, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:local-stt|inspect_custom_model_path", {
          path,
        }),
      };
    } catch (e) {
  ...
  async startServerForPath(path: string): Promise<Result<string, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:local-stt|start_server_for_path", {
          path,
        }),
      };
    } catch (e) {
...
export type CustomSttModelInfo = {
  path: string;
  name: string;
  sizeBytes: number;
  format: CustomSttModelFormat;
};
