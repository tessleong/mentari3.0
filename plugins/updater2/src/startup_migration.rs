...
fn legacy_target_app_path(current_app_path: &Path) -> Option<PathBuf> {
    let target_name = match current_app_path.file_name().and_then(|name| name.to_str()) {
        Some("Hyprnote.app") | Some("Char.app") => "Mentari.app",
        Some("Hyprnote Nightly.app") | Some("Char Nightly.app") => "Mentari Nightly.app",
        Some("Hyprnote Staging.app") | Some("Char Staging.app") => "Mentari Staging.app",
        _ => return None,
    };
...
mod tests {
    ...
    #[test]
    fn maps_legacy_bundle_names_to_mentari_names() {
        let cases = [
            ("/Applications/Hyprnote.app", "/Applications/Mentari.app"),
            ("/Applications/Char.app", "/Applications/Mentari.app"),
            (
                "/Applications/Hyprnote Nightly.app",
                "/Applications/Mentari Nightly.app",
            ),
            (
            ...
            (
                ...
                "/Applications/Mentari Staging.app",
            ),
            (
    ...
    fn rename_command_does_not_open_existing_target_bundle() {
        ...
        let command = build_bundle_rename_command(
            ...
            Path::new("/Applications/Mentari Nightly.app"),
            &relaunch_args,
        );
        let args = command
            .get_args()
        ...
        assert!(args[1].contains("if [ -e '/Applications/Mentari Nightly.app' ]; then"));
    ...
    fn current_bundle_path_from_executable_uses_bundle_root() {
        let executable = Path::new("/Applications/Mentari.app/Contents/MacOS/mentari");

        let bundle = current_app_bundle_path_from_executable(executable).unwrap();

        assert_eq!(bundle, PathBuf::from("/Applications/Mentari.app"));
    }
    ...
    fn rename_command_relaunches_from_target_bundle() {
        ...
        assert!(args[1].contains(
            "mv -f '/Applications/Hyprnote Nightly.app' '/Applications/Mentari Nightly.app'"
        ));
        assert!(args[1].contains(
            "open -n '/Applications/Mentari Nightly.app' --args '--onboarding=123' '--updater2-skip-startup-migration=1'"
        ));
    ...
    fn rename_command_relaunches_stable_bundle_with_skip_flag() {
        ...
        let command = build_bundle_rename_command(
            ...
            Path::new("/Applications/Mentari.app"),
            &relaunch_args,
        );
        let args = command
            .get_args()
        ...
        assert!(args[1].contains("mv -f '/Applications/Char.app' '/Applications/Mentari.app'"));
        assert!(args[1].contains(
            "open -n '/Applications/Mentari.app' --args '--updater2-skip-startup-migration=1'"
        ));
    }
}
