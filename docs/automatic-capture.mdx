...
Mentari can start an event-backed note at its scheduled time, remind you when another meeting app uses your microphone, and stop listening when the call ends.

## Start scheduled meetings automatically

Open **Settings → Meetings** and enable **Start when meeting begins**.

When a calendar-backed note reaches its scheduled start time, Mentari starts listening. This setting does not apply to ordinary notes without a calendar event.

Keep Mentari running and make sure microphone and system-audio access are allowed. See [Connect a calendar](/calendar) if your events are not visible.

## Detect unscheduled calls

Open **Settings → Notifications** and enable **Microphone detection**. Mentari watches for sustained microphone activity and notifies you that a meeting may have started.
...
Under **Settings → Meetings**, enable **Stop when meeting ends**. Mentari stops listening when the meeting app releases the microphone.
...
Enable **Show floating bar** under **Settings → Meetings** to keep compact recording controls visible while Mentari listens.
