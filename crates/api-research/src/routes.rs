...
async fn web_search(
    ...
    let response = state
        .exa
        .search(anlg_exa::SearchRequest {
            query: query.clone(),
            additional_queries: None,
            stream: None,
            output_schema: None,
            system_prompt: None,
            r#type: Some(anlg_exa::SearchType::Auto),
            category: None,
            user_location: None,
            ...
            contents: Some(anlg_exa::ContentsRequest {
                text: Some(anlg_exa::TextRequest::Options(anlg_exa::TextOptions {
                    max_characters: Some(1200),
                    include_html_tags: Some(false),
                    verbosity: Some(anlg_exa::TextVerbosity::Compact),
                    include_sections: None,
                    exclude_sections: None,
                })),
                highlights: Some(anlg_exa::HighlightsRequest::Options(
                    anlg_exa::HighlightsOptions {
                ...
                summary: Some(anlg_exa::SummaryRequest {
                    query: Some(format!("Summarize the information relevant to: {query}")),
                    schema: None,
                }),
                livecrawl: Some(anlg_exa::Livecrawl::Preferred),
...
fn normalize_search_result(result: anlg_exa::SearchResult) -> WebSearchResult {
    let snippet = result
        .summary
        .as_deref()
...
mod tests {
    ...
    fn normalize_search_result_prefers_summary_over_highlights() {
        let result = normalize_search_result(anlg_exa::SearchResult {
