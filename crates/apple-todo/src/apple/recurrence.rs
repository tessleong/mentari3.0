...
use objc2_event_kit::{
    EKCalendarItem, EKRecurrenceDayOfWeek, EKRecurrenceEnd, EKRecurrenceFrequency,
    EKRecurrenceRule, EKWeekday,
};
use objc2_foundation::{NSArray, NSDate, NSInteger, NSNumber};

use crate::types::{
    RecurrenceDayOfWeek, RecurrenceEnd, RecurrenceFrequency, RecurrenceRule, Weekday,
};

pub fn parse_recurrence_rules(item: &EKCalendarItem) -> Vec<RecurrenceRule> {
    unsafe {
        let rules: Option<Retained<NSArray<EKRecurrenceRule>>> = msg_send![item, recurrenceRules];
        rules
