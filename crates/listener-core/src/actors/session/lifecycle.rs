...
pub(crate) fn configure_sentry_session_context(params: &SessionParams) {
    sentry::configure_scope(|scope| {
        scope.set_tag("anarlog.session.id", &params.session_id);
        scope.set_tag(
        ...
        session_context.insert(
            "anarlog.session.id".to_string(),
            params.session_id.clone().into(),
        );
        session_context.insert(
            "anarlog.gen_ai.request.model".to_string(),
            params.model.clone().into(),
        );
        session_context.insert(
            "anarlog.session.transcription_mode".to_string(),
            format!("{:?}", params.transcription_mode).into(),
        );
        session_context.insert(
            "anarlog.session.onboarding".to_string(),
            params.onboarding.into(),
        );
        session_context.insert(
            "anarlog.session.language_codes".to_string(),
...
pub(crate) fn clear_sentry_session_context() {
    sentry::configure_scope(|scope| {
        scope.remove_tag("anarlog.session.id");
        scope.remove_tag("anarlog.session.type");
        scope.remove_context("anarlog.session");
    });
}
