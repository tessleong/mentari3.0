CREATE TRIGGER IF NOT EXISTS e2ee_dirty_transcripts_insert
...
BEGIN
  ...
  VALUES (NEW.workspace_id, 'transcripts', NEW.id)
...
CREATE TRIGGER IF NOT EXISTS e2ee_dirty_transcripts_delete
...
BEGIN
  INSERT INTO e2ee_dirty_rows (workspace_id, table_name, row_id)
  VALUES (OLD.workspace_id, 'transcripts', OLD.id)
  ON CONFLICT (workspace_id, table_name, row_id) DO UPDATE SET
    generation = e2ee_dirty_rows.generation + 1;
END;
