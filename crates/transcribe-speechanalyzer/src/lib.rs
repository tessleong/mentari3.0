...
pub struct FileTranscript {
    ...
    #[serde(default)]
    pub words: Vec<Word>,
    // Populated by the batch pipeline for single-channel (in-person)
    // recordings, where per-channel attribution can't tell speakers apart —
    // see `DiarizationSegment`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub speaker_segments: Vec<DiarizationSegment>,
}

// Mirrors `anlg_transcribe_soniqo::DiarizationSegment` field-for-field. Kept
// as a separate type rather than a shared dependency so this crate's only
// diarization-model dependency stays confined to the batch orchestrator
// (listener2-core), which already depends on both provider crates.
...
// A word is attributed to whichever segment contains its midpoint — using
// the midpoint rather than the start avoids misattributing a word that
// starts a hair before a speaker change actually lands acoustically.
pub fn speaker_for_word(
    ...
    segments
        .iter()
        .find(|segment| segment.start_seconds <= midpoint && midpoint < segment.end_seconds)
...
pub fn batch_response_from_transcripts(channels: Vec<FileTranscript>) -> batch::Response {
    ...
    let metadata = metadata_json(duration_seconds, channels.len() as u32, has_diarization);
        ...
        results: batch::Results {
            channels: channels
                ...
                .map(|(channel_index, channel)| {
                    ...
                    let speaker_segments = channel.speaker_segments;
                    let words = channel
                        .words
                        .into_iter()
                        .map(|word| {
                            let end = word.end.max(word.start + MIN_SYNTHETIC_DURATION_SECONDS);
                            let speaker = speaker_for_word(&speaker_segments, word.start, end);
                            batch::Word {
                                word: word.text.clone(),
                                start: word.start,
                                end,
...
fn metadata_json(duration_seconds: f64, channels: u32, has_diarization: bool) -> serde_json::Value {
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert("diarized".to_string(), serde_json::json!(has_diarization));
    }
    value
}
