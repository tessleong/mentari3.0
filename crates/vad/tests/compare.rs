...
fn silero_onnx_mask(audio: &[u8]) -> (Vec<bool>, f32) {
    ...
    for (i, chunk) in samples_f32.chunks(CHUNK_SIZE_16KHZ).enumerate() {
        if chunk.len() == CHUNK_SIZE_16KHZ {
            let view = anlg_onnx::ndarray::ArrayView1::from(chunk);
...
fn compare_all_engines() {
    compare("english_1", anlg_data::english_1::AUDIO);
    compare("english_2", anlg_data::english_2::AUDIO);
    compare("english_3", anlg_data::english_3::AUDIO);
    compare("korean_1", anlg_data::korean_1::AUDIO);
    compare("korean_2", anlg_data::korean_2::AUDIO);
}
