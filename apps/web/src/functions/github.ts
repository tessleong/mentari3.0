...
const GITHUB_ORG_REPO = "evitxchi/mentari2.0";
...
async function fetchGitHub(url: string, accept?: string): Promise<Response> {
  return fetchWithCache(url, { headers: getGitHubHeaders(accept) });
}
...
async function fetchGitHubStatsFromRepoPage(): Promise<GitHubStats | null> {
  try {
    ...
    const forksHrefPattern = new RegExp(
      `href="\\/${GITHUB_ORG_REPO.replace("/", "\\/")}\\/forks"[\\s\\S]{0,250}?<strong>([0-9,]+)<\\/strong>`,
    );
