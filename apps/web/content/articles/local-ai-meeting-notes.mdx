...
| **Mentari**       | Users who want a local-first meeting workflow and AI-provider choice | Free (BYOK), Pro $15/mo or $150/year |
...
[Mentari](https://anarlog.so) is an open-source AI notepad for meetings, built for people who want complete control over their data and AI stack. It captures system audio from any meeting platform without joining your call and without requiring calendar permissions.

You can take notes during the meeting in Mentari's built-in editor while it transcribes in the background, and the AI combines both into a structured summary after the call.

The AI stack is yours to choose: Mentari's managed cloud service, your own API keys, or fully local models through Ollama or LM Studio for offline operation. The canonical meeting record stays in local SQLite; attachments and recordings remain local files, and you decide whether to enable cloud services.
...
Mentari is the strongest choice if you want complete control: open-source, your choice of AI provider, and data that lives on your device in a format you own.
...
If you want to start with local AI meeting notes that give you full ownership, [try Mentari](https://anarlog.so). It is free, open-source, and works offline with local models. Your first meeting's notes will land on your device, readable by any tool you already use.
...
Yes. Tools that capture microphone input (Mentari, Meetily, Talat, Screenpipe) work for in-person conversations. Place your laptop near the speakers and let the mic pick up the room. Speaker diarization accuracy varies since it was primarily designed for virtual calls with separate audio channels.

### 3. Do local meeting notetakers work offline?

If both transcription and summarization use local models, yes. Mentari with Ollama, Meetily with Parakeet + Ollama, Talat with its built-in LLM, and Screenpipe with local Whisper all work without an internet connection. Shadow's transcription works offline but the AI summary features need a connection.
