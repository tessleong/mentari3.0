...
| --------- | -------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------ |
| Mentari  | Zero lock-in, complete control over data and AI stack    | Free forever (local + BYOK). Cloud: $15/mo        | All platforms (system audio capture) + in-person |
...
Mentari is an open-source AI notepad for meetings built for high-agency people who demand complete control over their data and AI stack. Meeting data is stored locally in SQLite instead of a vendor-hosted workspace. Markdown is available when you want to export it. It works like Apple Notes with AI superpowers, capturing audio from any meeting platform without joining as a bot.

<Image src="/api/assets/blog/fireflies-ai-alternatives/hyprnote.webp" alt="Mentari"/>

#### How it compares to Fireflies

Fireflies uses a bot to join meetings. Mentari captures system audio directly and keeps meeting data in local SQLite by default. You can choose an AI stack and export Markdown when you need a portable record.
...
If you like the idea of AI meeting notes but hate the idea of giving a vendor permanent leverage over your meeting data, start with Mentari.
...
[**Download Mentari for macOS**](/download) if that is the kind of system you would rather build on.
