...
- **Choose Mentari** when you want bot-free capture, local transcription, and a local-first meeting record on your own computer.
...
| Mentari | Desktop audio capture and local transcription | No | Local SQLite with Markdown export | Local-first, bot-free meeting notes |
...
Mentari is our open-source, local-first meeting notetaker for macOS. It captures the meeting from your computer, transcribes locally, and stores notes as Markdown on disk.

Because capture happens on your device, Mentari does not need to join the Teams call as a guest participant. It can be used with the Teams desktop or web experience without adding another name to the attendee list.
...
![Mentari desktop quick-start screen with New Note, Start Recording, and Settings actions](/api/assets/blog/library/products/anarlog/desktop/2026-07-22-quick-start.jpg)
...
Mentari is the strongest fit when "private" means controlling the artifact itself: where the transcript lives, which model sees it, and how the notes are retained.
...
### Choose Mentari when ownership comes first

Use Mentari if you want bot-free capture, local transcription, a local meeting record, and control over the AI model used for summaries.
...
Yes. A local desktop recorder such as Mentari captures from your device, while a browser-extension workflow such as Tactiq operates inside Teams on the web. Neither requires an automated guest participant.
...
[Download Mentari for macOS](/download) if you want Microsoft Teams notes without a meeting bot, with local transcription and meeting data stored on your computer.
