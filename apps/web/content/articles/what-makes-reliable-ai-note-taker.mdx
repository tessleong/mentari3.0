...
When we started building Mentari, we knew what we were up against. Otter, Fireflies, Granola—the space was already crowded with well-funded players offering impressive features.
...
## How we're building Mentari

We're designing Mentari around dependency thinking.
...
**Your choice of AI stack.** Mentari works fully offline out of the box, but you're not locked into local-only. Use the managed cloud service for the easiest setup, bring your own API keys (OpenAI, Anthropic, Mistral, and others), or run local models via Ollama or LM Studio. Switch anytime. No lock-in.

**Deployment clarity.** Mentari is open source and stores meeting data locally by default. That makes the desktop data path inspectable; it does not imply a supported self-hosted or private-cloud deployment.

**Direct audio capture.** Mentari captures system audio and microphone input directly at the OS level. No bots. No platform permissions. No dependency on Zoom or Teams policies. Works for all meeting platforms and in-person conversations.
