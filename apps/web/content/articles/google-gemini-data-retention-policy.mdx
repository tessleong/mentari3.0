...
## Use Gemini's API for Meeting Notes Through Mentari

If you want to use Gemini without routing data through the consumer product, connecting your own Google AI API key through [Mentari](https://anarlog.so) gives you API-level data handling: 55-day retention, no model training, no Gmail integration.

Mentari is an open-source AI notepad for meetings that lets you bring your own API key for Gemini, OpenAI, Anthropic, Mistral, or others. Your meeting notes are stored on your device. You choose which AI provider processes your data, and you can change that decision without rebuilding your workflow.

For teams that need to go further, Mentari supports fully local models via Ollama. Your conversations never leave your device at all.
