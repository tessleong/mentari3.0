...
pub fn main() {
    ...
    // shows "Mentari is already starting" on every launch and never serves dumps.
...
fn startup_failure_message(error: &impl std::fmt::Display) -> String {
    format!("Mentari failed to start: {error}")
}
...
fn exit_after_startup_failure(identifier: &str, error: &impl std::fmt::Display) -> ! {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let alert = if db::is_transient_lock_error(error) {
            "display alert \"Mentari is not ready yet\" message \"Another Mentari process is still using your data, possibly finishing an update. Your existing data was left unchanged. Please wait a moment and open Mentari again.\" as critical buttons {\"OK\"} default button \"OK\""
        } else if db::is_newer_schema_error(error) {
            "display alert \"Mentari needs an update\" message \"Your data was created by a newer version of Mentari, and this older version cannot open it. Your existing data was left unchanged. Please install the latest version of Mentari.\" as critical buttons {\"OK\"} default button \"OK\""
        } else {
            "display alert \"Mentari could not start\" message \"Your existing data was left unchanged. Please restart the app. If the problem continues, contact support.\" as critical buttons {\"OK\"} default button \"OK\""
        };
...
mod test {
    ...
    fn startup_failure_message_includes_the_original_error() {
        ...
        assert_eq!(
            message,
            "Mentari failed to start: legacy import did not pass parity verification"
        );
    }
