...
fn finds_windows_path_entries_case_insensitively() {
    let expected = Path::new(r"C:\Users\Test\AppData\Local\Mentari\bin");

    assert!(path_list_contains(
        r"C:\Windows;C:\USERS\TEST\APPDATA\LOCAL\MENTARI\BIN\;C:\Tools",
        expected
    ));
    assert!(!path_list_contains(
...
fn classifies_legacy_app_resource_symlink_as_missing() {
    ...
    let app_resource_path = dir
        .path()
        .join("Mentari.app/Contents/Resources/anarlog-cli");
    let install_path = dir.path().join("anarlog");
...
fn classifies_legacy_app_executable_symlink_as_missing() {
    ...
    let app_executable_path = dir.path().join("Mentari.app/Contents/MacOS/anarlog-cli");
    let install_path = dir.path().join("anarlog");
...
fn installed_cli_survives_bundled_resource_move() {
    let dir = tempfile::tempdir().unwrap();
    let resource_path = dir.path().join("Mentari.app/Contents/MacOS/anarlog-cli");
    ...
    install_managed_cli(&resource_path, &managed_path, &install_path).unwrap();
    std::fs::remove_dir_all(dir.path().join("Mentari.app")).unwrap();
