...
import { participantRolePromptLabel } from "~/session/participant-roles";
...
async function transformArgs(
  ...
  const { sessionId, enhancedNoteId, oneOffInstruction } = args;
  const snapshot = await loadSessionContentSnapshot(sessionId);
  if (!snapshot) {
    throw new Error(`Session ${sessionId} no longer exists`);
  }

  // A caller that's regenerating an existing note can lose track of its
  // template (e.g. pass null/undefined meaning "no override"). Falling back
  // to the note's own stored templateId keeps regeneration from silently
  // losing its template context — which otherwise disables the length-cap
  // bypass in enhance-success.ts and truncates the result unexpectedly.
  const existingNote = snapshot.enhancedNotes.find(
    (note) => note.id === enhancedNoteId,
  );
  const templateId = args.templateId || existingNote?.templateId || undefined;
  ...
  return {
    ...
    oneOffInstruction: oneOffInstruction?.trim() || null,
  };
}
...
function getParticipants(snapshot: SessionContentSnapshot): Participant[] {
  return snapshot.participants
    ...
    .map((participant) => ({
      ...
      role: participantRolePromptLabel(participant.role),
    }));
}
