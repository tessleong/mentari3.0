...
const mocks = vi.hoisted(() => ({
  ...
  getScheme: vi.fn().mockResolvedValue("mentari"),
  getSession: vi.fn(),
...
function SessionProbe() {
  ...
  return (
    <>
      ...
      <div data-testid="sign-in-modal-open">
        {isSignInModalOpen ? "open" : "closed"}
      </div>
      ...
      <button onClick={() => void signInWithOAuth("google").catch(() => {})}>
        Continue with Google
      </button>
      <button
        onClick={() =>
          void signInWithMagicLink("person@example.com").catch(() => {})
        }
      >
        Send magic link
      </button>
      <button
        onClick={() => void exchangeAuthCode("auth-code").catch(() => {})}
      >
        Exchange code
      </button>
    </>
...
describe("AuthProvider", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(openUrlWithInstruction).mockReset();
    ...
    mocks.getScheme.mockResolvedValue("mentari");
    ...
    mocks.signInWithOAuth.mockResolvedValue({
      data: { url: "https://project.supabase.co/auth/v1/authorize?x=1" },
      error: null,
    });
  ...
  it("rejects a different local database account before admission", async () => {
    ...
    expect(mocks.toastError).toHaveBeenCalledWith(
      "The notes on this device are linked to another Mentari account. Sign in with the account previously used here.",
  ...
  describe("native sign-in (no anarlog.so browser handoff)", () => {
    it("opens the in-app sign-in modal instead of a browser URL", async () => {
      renderAuthProvider();

      expect(screen.getByTestId("sign-in-modal-open").textContent).toBe(
        "closed",
      );
      ...
      expect(screen.getByTestId("sign-in-modal-open").textContent).toBe("open");
      expect(openUrlWithInstruction).not.toHaveBeenCalled();
    });

    it("starts Google OAuth against Supabase directly and opens the returned URL", async () => {
      renderAuthProvider();
      ...
      await waitFor(() => {
        expect(mocks.signInWithOAuth).toHaveBeenCalledWith({
          provider: "google",
          options: {
            redirectTo: "mentari://auth/callback",
            skipBrowserRedirect: true,
          },
        });
      });
      expect(openUrlWithInstruction).toHaveBeenCalledWith(
        "https://project.supabase.co/auth/v1/authorize?x=1",
        "sign-in",
        expect.any(Function),
      );
    });

    it("never builds a URL on anarlog.so for OAuth", async () => {
      renderAuthProvider();
      ...
      const [openedUrl] = vi.mocked(openUrlWithInstruction).mock.calls[0]!;
      expect(openedUrl).not.toContain("anarlog.so");
    });

    it("sends a magic link via Supabase OTP with the app's deep-link redirect", async () => {
      renderAuthProvider();
      ...
      await waitFor(() => {
        expect(mocks.signInWithOtp).toHaveBeenCalledWith({
          email: "person@example.com",
          options: { emailRedirectTo: "mentari://auth/callback" },
        });
      });
      expect(openUrlWithInstruction).not.toHaveBeenCalled();
    });

    it("exchanges a PKCE auth code for a session via Supabase", async () => {
      renderAuthProvider();
