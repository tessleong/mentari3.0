...
export function useAiProvidersState(type: AiProviderType): {
  ...
  // Stable reference when nothing has actually changed: this result feeds
  // useLLMConnection's memo -> useLanguageModel's memo -> the chat
  // transport's memo -> the AI SDK Chat instance's memo
  // (session-provider.tsx). An unmemoized object here breaks every memo
  // downstream, reconstructing a brand new Chat instance on every render —
  // which silently abandons any in-flight streaming request and drops the
  // "Thinking..." indicator, since renders happen constantly while a
  // response streams in.
