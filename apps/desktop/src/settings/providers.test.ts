...
describe("SQLite AI providers", () => {
  ...
  it("returns a stable providers reference across re-renders when nothing changed, so downstream memos (the chat model, transport, and AI SDK Chat instance) don't rebuild on every render", async () => {
    mocks.useLiveQuery.mockReturnValue({
      data: [
        {
