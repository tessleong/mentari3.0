...
function useConfiguredMapping(): {
  ...
  const soniqoDownloading = useQueries({
    queries: [...soniqoModels.map((m) => sttModelQueries.isDownloading(m.key))],
  });

  const appleSpeechDownloading = useQueries({
    queries: [
      ...appleSpeechModels.map((m) => sttModelQueries.isDownloading(m.key)),
    ],
  });

  const soniqoDownloaded = useQueries({
    queries: [
      ...soniqoModels.map((m, index) => ({
        ...sttModelQueries.isDownloaded(m.key),
        refetchInterval: soniqoDownloading[index]?.data
          ? 1000
          : (false as const),
      })),
    ],
  ...
  const appleSpeechDownloaded = useQueries({
    queries: [
      ...appleSpeechModels.map((m, index) => ({
        ...sttModelQueries.isDownloaded(m.key),
        refetchInterval: appleSpeechDownloading[index]?.data
          ? 1000
          : (false as const),
      })),
    ],
...
function buildOnDeviceModelEntries(
  ...
  activeDownloads: Array<{ data?: boolean }>,
  ...
  return models
    .map((model, index) => ({
      ...
      isDownloading: activeDownloads[index]?.data ?? false,
...
function ModelSelectItem({
  ...
  const isDownloading =
    model.isDownloading ||
    !!downloadInfo ||
    queuedDownloads.includes(model.id as LocalModel);
