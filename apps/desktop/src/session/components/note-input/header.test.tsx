...
describe("Header", () => {
  ...
  it("renders icon views and focuses summary before opening the template picker", () => {
    ...
    expect(memoTab.className).toContain("text-sky-500");
    expect(memoTab.className).toContain("shadow-xs");
    expect(memoTab.className).toContain("dark:text-sky-400");
    ...
    expect(activeSummaryTab.className).toContain("text-violet-500");
    expect(activeSummaryTab.className).toContain("dark:text-violet-400");
  ...
  it("renders the plain english tab and switches to it on click", () => {
    const handleTabChange = vi.fn();
    const editorTabs: EditorView[] = [
      { type: "enhanced", id: "note-1" },
      { type: "raw" },
    ...
    expect(handleTabChange).toHaveBeenCalledWith({ type: "plain_english" });
  });

  it("includes the plain english tab alongside the transcript tab", () => {
    const { result } = renderHook(() =>
      useEditorTabs({ sessionId: "session-1", audioExists: true }),
    );

    expect(result.current).toContainEqual({ type: "plain_english" });
  });
