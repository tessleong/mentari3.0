...
describe("SpeakerAssignPopover", () => {
  ...
  it("surfaces a speaker already assigned on this channel, for reconciling after a diarization index drift", async () => {
    useHumansMock.mockReturnValue([{ id: "human-1", name: "Evan", email: "" }]);
    useSessionParticipantsMock.mockReturnValue([
      { humanId: "human-1", name: "Evan", email: "" },
    ]);
    ...
    render(
      createElement(SpeakerAssignPopover, {
        // A new speaker_index (2) on the same RemoteParty channel that
        // already has an assignment (index 0) -- exactly what live
        // diarization renumbering looks like mid-session.
        segment: {
          id: "segment-1",
          key: {
            channel: "RemoteParty",
    ...
    fireEvent.click(screen.getByRole("button", { name: "Speaker 3" }));

    expect(screen.getByText("Already assigned on this channel")).toBeTruthy();
...
describe("buildSpeakerParticipantGroups", () => {
  ...
  it("lists already-assigned speakers first, ahead of participants and people", () => {
    const evan = option("human-1", "Evan", { isSessionParticipant: true });
    const groups = buildSpeakerParticipantGroups({
      sessionParticipants: [evan],
      contacts: [option("human-2", "Bob")],
      alreadyAssigned: [evan],
      query: "",
    });

    expect(groups).toEqual([
      { title: "Assigned", options: [evan] },
      { title: "People", options: [option("human-2", "Bob")] },
    ]);
  });

  it("does not list an already-assigned speaker twice", () => {
    const evan = option("human-1", "Evan", { isSessionParticipant: true });
    const groups = buildSpeakerParticipantGroups({
      sessionParticipants: [evan],
      contacts: [],
    ...
    expect(groups).toEqual([{ title: "Assigned", options: [evan] }]);
  });
});
