...
export const buildListMeetingsTool = () =>
  tool({
    description:
      "List recent Mentari meetings with pagination metadata. Use query to narrow by title or meeting id, then pass next_offset as offset to continue.",
...
export const buildGetMeetingTool = () =>
  tool({
    description:
      "Get one Mentari meeting with its canonical note, summaries, participants, and action items. Use get_meeting_transcript separately for transcript words.",
    inputSchema: z.object({
      meeting_id: z.string().describe("Mentari meeting id"),
    }),
    execute: getMeeting,
  });
...
export const buildGetMeetingTranscriptTool = () =>
  tool({
    description:
      "Get a bounded page of transcript words and readable text for an Mentari meeting. Pass pagination.next_offset as offset to continue.",
    inputSchema: z.object({
      meeting_id: z.string().describe("Mentari meeting id"),
      offset: z
        .number()
