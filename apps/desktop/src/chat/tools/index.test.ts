...
describe("chat tool registration", () => {
  ...
  it("registers meeting vocabulary without legacy shell-shaped names", () => {
    ...
    expect(tools).toHaveProperty("search_encounter_transcript");
    expect(tools).toHaveProperty("explain_medical_term");
    expect(tools).toHaveProperty("get_visit_next_steps");
