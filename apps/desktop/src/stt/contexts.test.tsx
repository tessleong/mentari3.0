...
import type { NotificationIconAsset } from "@anlg/plugin-notification";
...
function mentariReadyIcon(badge: {
  type: string;
  bundle_id?: string;
  path?: string;
}) {
  return {
    type: "overlay",
    base: { type: "app_icon" },
    badge: badge as NotificationIconAsset,
  };
}
    ...
    await vi.waitFor(() =>
      expect(showNotificationMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          icon: mentariReadyIcon(slackIcon),
        }),
      ),
    );
  });
  ...
  test.each([
    ...
    async (browser) => {
      ...
      expect(notification).toEqual({
        ...
        message: "Mentari will stop listening in 30 seconds.",
