...
describe("getSessionKeywords", () => {
  it("builds keywords from the current session snapshot", async () => {
    ...
    await expect(
      getSessionKeywords({
        sessionId: "session-1",
        dictionaryTerms: ["Mentari"],
      }),
    ).resolves.toEqual(expect.arrayContaining(["Mentari", "Launch"]));
  });
  ...
  it("prioritizes mapped participants and attached event attendees", async () => {
    ...
    expect(result.slice(0, 3)).toEqual(["Alice Kim", "Mina Park", "Mentari"]);
...
describe("dictionary term helpers", () => {
  it("parses stored JSON dictionary terms", () => {
    expect(
      parseDictionaryTermsJson(JSON.stringify(["Mentari", "Char"])),
    ).toEqual(["Mentari", "Char"]);
    expect(parseDictionaryTermsJson(["Mentari", " mentari "])).toEqual([
      "Mentari",
    ]);
  ...
  it("parses newline and comma separated terms", () => {
    expect(
      parseDictionaryTermsText("Mentari\nFastConformer, Parakeet TDT"),
    ).toEqual(["Mentari", "FastConformer", "Parakeet TDT"]);
  });

  it("normalizes duplicate terms while preserving first spelling", () => {
    expect(normalizeKeywordList(["Mentari", " mentari ", "Parakeet"])).toEqual([
      "Mentari",
      "Parakeet",
    ]);
  });
  ...
  it("formats stored terms one per line", () => {
    expect(formatDictionaryTerms(["Mentari", "Parakeet TDT"])).toBe(
      "Mentari\nParakeet TDT",
    );
  });
});
