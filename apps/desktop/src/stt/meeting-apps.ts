...
export function getMentariReadyNotificationIcon(
  detectedAppIcon: NotificationIcon | null,
): NotificationIcon {
  return {
    type: "overlay",
    base: { type: "app_icon" },
    badge: notificationIconAsset(detectedAppIcon) ?? {
      type: "system_symbol",
      name: "waveform",
    },
  };
...
function notificationIconAsset(
  icon: NotificationIcon | null,
): NotificationIconAsset | null {
  if (!icon || icon.type === "hidden" || icon.type === "overlay") {
    return null;
  }
