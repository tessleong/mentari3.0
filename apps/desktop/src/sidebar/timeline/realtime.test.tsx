...
describe("CurrentTimeIndicator", () => {
  ...
  test("uses primary current-time colors in light and dark mode", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2024, 0, 15, 12, 0, 0));
    ...
    expect(line?.className).toContain("dark:bg-primary/70");
    ...
    expect(label?.className).toContain("dark:border-primary");
    expect(label?.className).toContain("dark:bg-primary");
    expect(label?.className).toContain("dark:text-primary-foreground");
  });
